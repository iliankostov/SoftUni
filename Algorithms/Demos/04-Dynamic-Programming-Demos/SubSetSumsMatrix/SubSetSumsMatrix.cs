...
{
    private static int[] nums = { 5, 5, 15, 20, 1 };

    private static int sum = 26;

    private static bool[,] isCalculated = new bool[nums.Length, sum + 1];

    private static bool[,] possibleSum = new bool[nums.Length, sum + 1];
    ...
    private static bool CalcPossibleSums(int i, int sum)
    {
        ...
        {
            possibleSum[i, sum] = (nums[i] == sum) || CalcPossibleSums(i - 1, sum)
    ...
    private static void PrintSubset(int i, int sum)
    {
        ...
        while (true)
        {
            ...
            if (CalcPossibleSums(i - 1, sum - SubSetSums.nums[i]))
            {
                // Take arr[i]
                nums.Add(SubSetSums.nums[i]);
